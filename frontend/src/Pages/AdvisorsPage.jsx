...
import { AdvisorList, deleteUser, editUser } from '../features/UserSlice'


function AdvisorsPage() {

    const advisor = useSelector((state) => state.Users)
    ...
    const delete_user = (id) => {
        ...
        setAdvisors(prevList => prevList.filter(user => user.id !== id));
    }
    ...
    const searchItem = async (e) => {
        setFind(advisors)
        if (e.target.value === '') {
            return setAdvisors(find)
        } else {
            setAdvisors(await advisors.filter((item) => {
    ...
    useEffect(() => {
        dispatch(AdvisorList())
    }, [dispatch])

    useEffect(() => {
        if (advisor.state.length > 0) {
            return setAdvisors(advisor.state)
        }
    }, [advisor.state])
                        ...
                        <div className='grid items-start mx-5 justify-center mt-10'>
                            {
                                advisors.map((item) => {
                                    return (
                                        <div className='flex bg-[#23283B] mx-[50px]  my-5 p-2 rounded-3xl opacity-80 lg:min-w-[900px] md:min-w-[500px] sm:min-w-[350px] xs:min-w-[350px]'>
                                            <span className='rounded-full flex items-center justify-center min-w-[33px] min-h-[33px] bg-[#2E313D] '>
                                                {item.id}
                                            </span>
