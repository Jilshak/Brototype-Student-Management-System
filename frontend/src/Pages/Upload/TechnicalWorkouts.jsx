...
import { get_technical_tasks, removeTechnicalTask, technical_tasks } from '../../features/WeekDetails';
...
function TechnicalWorkouts() {
    ...
    useEffect(() => {
        ...
        dispatch(get_technical_tasks(credentials))
    }, [])

    const uploadFiles = async (e) => {
        console.log("This is the file: ", e)
        const data = new FormData()
        data.append('technical_tasks', e)

        let credentials = {
            week_id: id,
            userid: decode.user_id,
            technical_tasks: data
        }
        await Promise.resolve(dispatch(technical_tasks(credentials)))

        let credentials1 = {
            week_id: id,
            userid: decode.user_id
        }
        await Promise.resolve(dispatch(get_technical_tasks(credentials1)))
    }
    ...
    const removeFiles = async () => {
        ...
        await Promise.resolve(dispatch(removeTechnicalTask(credentials)))
                            ...
                            <button type="button" className="flex text-sm bg-gray-800 rounded-full">
                                <label htmlFor="this_week_task">
                                    <span className='bg-[#191C24] cursor-pointer py-5 flex items-center justify-center px-10 rounded-full text-lg opacity-80'>
                                        ...
                                        <p>Upload this technical Workouts here</p>
                                    </span>
                                    <input type="file" name='this_week_task' accept='application/msword,application/pdf' id='this_week_task' onChange={(e) => uploadFiles(e.target.files[0])} className='hidden' />
                                </label>
                            </button>
