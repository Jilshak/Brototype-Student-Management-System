...
import { get_personal_tasks, personal_tasks, removePersonalTasks } from '../../features/WeekDetails';
...
function PersonalWorkouts() {
    ...
    useEffect(() => {
        ...
        dispatch(get_personal_tasks(credentials))
    }, [])

    const uploadFiles = async (e) => {
        console.log("This is the file: ", e)
        const data = new FormData()
        data.append('personal_tasks', e)
        ...
        await Promise.resolve(dispatch(personal_tasks(credentials)))

        let credentials1 = {
            week_id: id,
            userid: decode.user_id
        }
        await Promise.resolve(dispatch(get_personal_tasks(credentials1)))
    }
    ...
    const removeFiles = async () => {
        ...
        await Promise.resolve(dispatch(removePersonalTasks(credentials)))
                            ...
                            <button type="button" className="flex text-sm bg-gray-800 rounded-full">
                                <label htmlFor="this_week_task">
                                    <span className='bg-[#191C24] cursor-pointer py-5 flex items-center justify-center px-10 rounded-full text-lg opacity-80'>
                                        ...
                                        <p>Upload this Personal Workouts here</p>
                                    </span>
                                    <input type="file" name='this_week_task' accept='application/msword,application/pdf' id='this_week_task' onChange={(e) => uploadFiles(e.target.files[0])} className='hidden' />
                                </label>
                            </button>
